...
**Де дивитися скейлінг HPA:** дашборд **«Retail Store — Replicas & CPU»** (uid: `retail-hpa-demo`). Зверху вибери змінну **Namespaces** → обери **All** або тільки staging: `carts-staging`, `ui-staging`, `catalog-staging`, `checkout-staging`, `orders-staging`. Графіки **«Replicas by Deployment (тут видно скейлінг HPA)»** та **«Ready Pods by Namespace (зростання = більше подів)»** — саме там під час навантаження (наприклад Locust) має зростати кількість реплік/подів. Якщо скейлінг не видно — переконайся, що в Namespaces обрано staging-неймспейси (а не лише prod).

**Помилка «the server could not find the requested resource (get pods.metrics.k8s.io)» / на графіку кількість реплік не змінюється:** HPA не бачить CPU, бо в кластері немає або не працює **metrics-server**. Зроби так:
1. Перевір наявність: `kubectl get deployment -n kube-system metrics-server` (або `kubectl get pods -n kube-system -l app.kubernetes.io/name=metrics-server`). Якщо нічого немає — встанови metrics-server.
2. **Через Terraform (рекомендовано):** у `terraform/lib/eks/eks.tf` вже є `enable_metrics_server = true` у модулі `eks_blueprints_addons`. Виконай `cd terraform/eks/staging && terraform apply` (або prod), щоб addon підтягнувся. Після apply зачекай 1–2 хв і перевір: `kubectl get pods -n kube-system -l app.kubernetes.io/name=metrics-server`.
3. **Без Terraform:** встанови вручну: `kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml`. На EKS може знадобитися додати до deployment `metrics-server` аргумент `--kubelet-use-node-status-port` та в container args: `- --kubelet-insecure-tls` (якщо ноди використовують самопідписані сертифікати). Після цього HPA почне отримувати метрики і кількість реплік на графіку зможе змінюватися.
4. Інші події («invalid capacity 0 on image filesystem», «Node Not Found» для Endpoint Slices) — часто проходять після того, як metrics-server запрацює і ноди стабілізуються; при потребі перезапусти ноди або пересинхронізуй застосунки в Argo CD.
