...
locals {
  ...
  gitops_dir_include  = var.gitops_target_revision == "staging" ? "{*staging*.yaml,monitoring*.yaml,monitoring-rules*.yaml}" : "*.yaml"
  ...
  provisioner "local-exec" {
    command     = <<-EOT
      ...
      if [ "$REV" = "staging" ]; then
        ...
        for f in "$DIR"/monitoring*.yaml "$DIR"/monitoring-rules*.yaml; do [ -e "$f" ] && kubectl apply -f "$f" || true; done
      else
